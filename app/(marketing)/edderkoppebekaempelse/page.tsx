...
export default function EdderkoppebekaempelsePage() {
  const [selectedPackage, setSelectedPackage] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);

  const service = services.find((s) => s.id === "edderkopper");
  ...
  const benefits = [
    "Fjerner effektivt spind og insekter",
    "Mindsker tiltrækning af nye edderkopper",
    "Usynlig og lugtfri behandling",
    "Holder facader, vinduer og udhæng pæne i lang tid",
    "Skånsom for både mennesker og materialer"
  ];
    ...
    <>
      <div className="relative min-h-screen pt-40 pb-16">
        <div className="absolute inset-0 -z-10">
          <Image
            src="/images/edderkop.png"
            alt="Edderkoppebekæmpelse baggrund"
            fill
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {/* Top section: Heading/text left, bullets right */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-stretch mb-16 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col justify-start space-y-6 motion-reduce:animate-none animate-slide-in-left">
              <h2 className="text-4xl font-bold text-white drop-shadow-lg leading-tight">
                Edderkoppebekæmpelse – slip for spind og insekter
              </h2>

              <p className="text-lg text-white/95 drop-shadow leading-relaxed">
                Edderkopper og spind samler sig hurtigt omkring udhæng, facader, vinduer og lamper. Med professionel edderkoppebekæmpelse reduceres både edderkopper og deres fødegrundlag, så dit hus forbliver pænt og frit for generende spind – uden brug af skadelige kemikalier.
              </p>
            </div>

            {/* Right column - Benefits bullets */}
            <div className="flex flex-col justify-start motion-reduce:animate-none animate-slide-in-right">
              <h3 className="text-2xl font-bold text-white drop-shadow-lg mb-4">Fordele ved edderkoppebekæmpelse:</h3>
          ...
          {/* Bottom section: Pricing cards full width */}
