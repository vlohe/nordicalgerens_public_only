...
export default function TagrenderensPage() {
  const [selectedPackage, setSelectedPackage] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);

  const service = services.find((s) => s.id === "tagrender");
  ...
  const benefits = [
    "Forebygger vandskader på facade og sokkel",
    "Sikrer korrekt afvanding fra taget",
    "Forhindrer frostsprængninger om vinteren",
    "Reducerer risiko for skadedyr i tagrender",
    "Giver tryghed året rundt uden bekymringer"
  ];
    ...
    <>
      <div className="relative min-h-screen pt-40 pb-16">
        <div className="absolute inset-0 -z-10">
          <Image
            src="/images/tagrende.png"
            alt="Tagrenderens baggrund"
            fill
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-stretch mb-16 max-w-6xl mx-auto">
            <div className="flex flex-col justify-start space-y-6 motion-reduce:animate-none animate-slide-in-left">
              <h2 className="text-4xl font-bold text-white drop-shadow-lg leading-tight">
                Tagrenderens – undgå fugtskader og oversvømmelser
              </h2>

              <p className="text-lg text-white/95 drop-shadow leading-relaxed">
                Tilstoppede tagrender kan føre til alvorlige skader på facade, sokkel og fundament. Med professionel tagrenderens fjernes blade, skidt og mos, så vandet igen kan løbe frit – og dit hus er beskyttet mod dyre følgeskader.
              </p>
            </div>

            <div className="flex flex-col justify-start motion-reduce:animate-none animate-slide-in-right">
              <h3 className="text-2xl font-bold text-white drop-shadow-lg mb-4">Fordele ved tagrenderens:</h3>
